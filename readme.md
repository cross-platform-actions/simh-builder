...
[`.github/workflows/ci.yml`](.github/workflows/ci.yml) and to the above
...
2. Run `build.sh` to build a simulator:
    ...
    By default a pinned Open SIMH commit is built (see `simh_revision` in
    [`build.sh`](build.sh)). Open SIMH does not publish releases or tags, so
    the commit is pinned for reproducible builds and bumped deliberately. To
    build a different commit, branch or tag, set the `SIMH_REVISION`
    environment variable:

    ```
    SIMH_REVISION=master ./build.sh vax
    ```
